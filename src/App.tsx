...
const SterkeErvenLogo = ({ className = "", onClick }: { className?: string; onClick?: () => void }) => {
  const textSize = "text-3xl md:text-5xl";
  return (
    <div 
      onClick={onClick}
      className={`flex flex-col gap-[2px] leading-none select-none ${onClick ? 'cursor-pointer' : ''} ${className}`}
    >
      <div className="flex">
        <span className={`font-black text-se-yellow ${textSize} tracking-tighter uppercase`}>STERKE</span>
      </div>
      <div className="flex">
        {/* The transparent 'ST' ensures 'ERVEN' starts exactly where 'ERKE' starts, aligning the 'E's */}
        <span className={`font-black text-transparent ${textSize} tracking-tighter uppercase select-none`}>ST</span>
        <span className={`font-black text-se-yellow ${textSize} tracking-tighter uppercase`}>ERVEN</span>
      </div>
    </div>
  );
};
...
export default function App() {
  ...
  const [marketingGoal, setMarketingGoal] = useState<Goal | null>(null);
  const [audienceFocus, setAudienceFocus] = useState<'decision_makers' | 'influencers' | null>(null);
  const [campaignType, setCampaignType] = useState<'launch' | 'maintenance' | 'event' | null>(null);
  ...
  const handleDownloadPDF = async () => {
    const doc = new jsPDF();
    const primaryColor: [number, number, number] = [0, 86, 106]; // #00566a (se-teal)
    const accentColor: [number, number, number] = [255, 237, 0]; // #ffed00 (se-yellow)
    ...
    doc.text('STERKE', 15, 20);
    doc.text('  ERVEN', 15, 32); // Indented to align E's
    
    doc.setTextColor(255, 255, 255);
    doc.setFontSize(18);
    doc.text('MEDIA VOORSTEL', 15, 42);
    
    doc.setFontSize(10);
    doc.text(`Datum: ${new Date().toLocaleDateString('nl-NL')}`, 150, 42);

    // Summary Section
    doc.setTextColor(0, 0, 0);
    doc.setFontSize(14);
    doc.text('Samenvatting Campagne', 15, 65);
    ...
    autoTable(doc, {
      ...
      body: [
        ...
        ['Marketing Doel', marketingGoal?.toUpperCase() || 'N/A'],
        ['Looptijd', `${duration} weken`],
      ],
      theme: 'striped',
      headStyles: { fillColor: primaryColor, textColor: [255, 255, 255] },
      alternateRowStyles: { fillColor: [245, 243, 219] }, // se-beige approx
    });
    ...
    for (const rec of recommendations) {
      ...
      doc.rect(15, currentY + 5, 5, 10, 'F');
      
      doc.setFontSize(12);
      doc.setFont('helvetica', 'bold');
      doc.setTextColor(primaryColor[0], primaryColor[1], primaryColor[2]);
      doc.text(rec.sector.name.toUpperCase(), 25, currentY + 13);
      
      currentY += 20;

      // Add sector image
      try {
        const imgData = await getBase64Image(rec.sector.image);
        doc.addImage(imgData, 'JPEG', 15, currentY, 60, 35);
        currentY += 40;
      } catch (e) {
  ...
  const handleSendQuote = async (e: React.FormEvent) => {
    ...
    console.log('Offerte verstuurd naar niels@agrio.nl', {
      userEmail,
      userPhone,
      ...
      <header className="fixed top-0 left-0 right-0 bg-white/80 backdrop-blur-xl border-b border-se-dark/5 z-50">
        <div className="max-w-7xl mx-auto px-4 md:px-8 h-16 md:h-20 flex items-center justify-between">
          <div className="flex items-center">
            <SterkeErvenLogo onClick={() => setCurrentStep(0)} className="scale-[0.4] md:scale-[0.5] origin-left" />
          </div>
        ...
        <AnimatePresence mode="wait">
            ...
            >
              <div className="relative">
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                  <SterkeErvenLogo className="scale-[0.35] md:scale-[0.45]" />
                </div>
              </div>

              <div className="space-y-6 max-w-md w-full">
                <div className="space-y-2">
                  <h3 className="text-2xl md:text-4xl font-black uppercase tracking-tighter bg-transparent">Plan wordt samengesteld</h3>
                  <p className="text-se-teal font-black text-xs uppercase tracking-widest h-4 bg-transparent">
                    {calculationMessage}
                  </p>
                </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-4 overflow-y-auto pr-2">
                    {AGRIO_SECTORS.map(sector => (
                      ...
                      >
                        <img 
                          src={sector.image} 
                          className={`absolute inset-0 w-full h-full object-cover transition-transform duration-700 group-hover:scale-110 ${selectedSectors.includes(sector.id) ? 'opacity-80' : 'opacity-30 grayscale'}`}
                          alt={sector.name}
                          referrerPolicy="no-referrer"
                        />
          ...
          {!isCalculating && currentStep === 7 && (
            ...
            >
              ...
              <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-6 border-b-4 md:border-b-8 border-se-dark pb-8 md:pb-12">
                ...
                <div className="flex flex-col sm:flex-row gap-3 w-full lg:w-auto">
                  <button 
                    ...
                    className="flex-1 lg:flex-none flex items-center justify-center gap-2 px-8 py-5 bg-se-yellow font-black uppercase tracking-widest hover:bg-se-dark hover:text-white transition-all text-xs md:text-sm shadow-xl shadow-se-yellow/20 text-se-dark"
                  >
                    <Mail className="w-5 h-5" /> Mail Offerte
                  </button>
                  <button 
                    onClick={handleDownloadPDF}
                    className="flex-1 lg:flex-none flex items-center justify-center gap-2 px-8 py-5 border-2 border-se-dark bg-white font-black uppercase tracking-widest hover:bg-se-dark hover:text-white transition-all text-xs md:text-sm"
                  >
              ...
              {/* Plan Summary Card */}
              <div className="bg-se-dark text-white p-8 md:p-12 space-y-6 relative overflow-hidden">
                <div className="absolute top-0 right-0 w-64 h-64 bg-se-yellow/10 rounded-full -mr-32 -mt-32 blur-3xl" />
                <div className="relative z-10 grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
                  <div className="space-y-4">
                    <h4 className="text-2xl md:text-4xl font-black uppercase tracking-tight">Plan Samenvatting</h4>
                    <p className="text-white/60 font-medium leading-relaxed">
                      Dit voorstel is geoptimaliseerd voor <span className="text-se-yellow">{marketingGoal === 'branding' ? 'Naamsbekendheid' : marketingGoal === 'expertise' ? 'Thought Leadership' : marketingGoal === 'leads' ? 'Lead Generatie' : 'Product Introductie'}</span> binnen de <span className="text-se-yellow">{selectedSectors.map(id => AGRIO_SECTORS.find(s => s.id === id)?.name).join(', ')}</span> sector.
                    </p>
                  </div>
                  <div className="bg-white/5 p-6 border border-white/10 space-y-4">
                    <div className="flex justify-between items-center border-b border-white/10 pb-2">
                      <span className="text-[10px] font-black uppercase tracking-widest opacity-40">Focus Doelgroep</span>
                      <span className="text-sm font-black uppercase">{audienceFocus === 'decision_makers' ? 'Beslissers' : 'Beïnvloeders'}</span>
                    </div>
                    <div className="flex justify-between items-center border-b border-white/10 pb-2">
                      <span className="text-[10px] font-black uppercase tracking-widest opacity-40">Campagne Type</span>
                      <span className="text-sm font-black uppercase">{campaignType === 'launch' ? 'Introductie' : campaignType === 'maintenance' ? 'Onderhoud' : 'Evenement'}</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-[10px] font-black uppercase tracking-widest opacity-40">Verwachte Impact</span>
                      <span className="text-sm font-black uppercase text-se-yellow">Hoog</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-se-dark text-white p-8 md:p-24 text-center space-y-8 md:space-y-12 relative overflow-hidden">
                ...
                <div className="relative z-10 space-y-6 md:space-y-8">
                  <h3 className="text-3xl md:text-7xl font-black tracking-tighter uppercase leading-none">Bouw jij mee aan <br /> <span className="text-se-yellow">sterke erven?</span></h3>
                  ...
                  <div className="flex flex-col sm:flex-row flex-wrap gap-4 justify-center pt-4 md:pt-8">
                    <button 
                      onClick={() => setIsQuoteModalOpen(true)}
                    ...
                    <button 
                      onClick={handleDownloadPDF}
                      className="bg-white text-se-dark px-8 py-5 md:px-12 md:py-6 font-black uppercase tracking-widest text-sm md:text-lg hover:scale-105 transition-transform shadow-2xl shadow-white/10 flex items-center justify-center gap-3"
                    >
                      <Download className="w-5 h-5" /> Download PDF
                    </button>
                    <a 
                      href="tel:+31314626434"
                      className="px-8 py-5 md:px-12 md:py-6 border-2 border-white/20 font-black uppercase tracking-widest text-sm md:text-lg hover:bg-white/5 transition-all flex items-center justify-center gap-3"
                    >
                      <Phone className="w-5 h-5" /> Bel Agrio
                    </a>
        ...
        {isQuoteModalOpen && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-6">
            <motion.div 
              ...
              className="absolute inset-0 bg-se-dark/80 backdrop-blur-sm"
            />
            <motion.div 
            ...
            >
              <div className="p-6 md:p-10 space-y-8">
                <div className="flex justify-between items-start">
                  <div className="space-y-2">
                    <h4 className="text-2xl md:text-3xl font-black uppercase tracking-tight">Ontvang uw <span className="text-se-teal">offerte</span></h4>
                    <p className="text-sm text-se-dark/60 font-medium">Laat uw gegevens achter en wij sturen het volledige mediavoorstel naar niels@agrio.nl</p>
                  </div>
                  <button 
                    onClick={() => setIsQuoteModalOpen(false)}
                    className="p-2 hover:bg-se-dark/5 transition-colors"
                  >
                    <X className="w-6 h-6" />
                  </button>
                </div>
                ...
                {isSent ? (
                  ...
                  >
                    <div className="w-20 h-20 bg-se-teal rounded-full flex items-center justify-center mx-auto mb-6">
                      <Check className="w-10 h-10 text-white" />
                    </div>
                    <h5 className="text-2xl font-black uppercase tracking-tight">Bedankt!</h5>
                    <p className="text-se-dark/60 font-medium">De offerte is onderweg naar Agrio.</p>
                  </motion.div>
                ) : (
                  <form onSubmit={handleSendQuote} className="space-y-6">
                    <div className="space-y-4">
                      <div className="space-y-2">
                        <label className="text-[10px] font-black uppercase tracking-widest opacity-40">E-mailadres</label>
                        <input 
                          required
                          type="email"
                          value={userEmail}
                          ...
                          className="w-full p-4 bg-se-dark/5 border-2 border-transparent focus:border-se-teal outline-none transition-all font-medium"
                        />
                      </div>
                      <div className="space-y-2">
                        <label className="text-[10px] font-black uppercase tracking-widest opacity-40">Telefoonnummer</label>
                        <input 
                          required
                          type="tel"
                          value={userPhone}
                    ...
                    <button 
                      type="submit"
                      disabled={isSending}
                      className={`w-full p-5 font-black uppercase tracking-widest flex items-center justify-center gap-3 transition-all ${
                        isSending ? 'bg-se-dark/10 text-se-dark/20 cursor-not-allowed' : 'bg-se-dark text-white hover:bg-se-teal'
                      }`}
                    >
                      {isSending ? (
                        <>Versturen...</>
                        ...
                        <>
                          <Send className="w-5 h-5" /> Offerte Aanvragen
                        </>
                      )}
                    </button>
                    
                    <p className="text-[10px] text-center text-se-dark/40 font-medium italic">
          ...
          <div className="max-w-4xl mx-auto flex gap-3">
            ...
            <button 
              ...
              disabled={
                ...
                (currentStep === 3 && audienceFocus === null) ||
                (currentStep === 4 && marketingGoal === null) ||
                (currentStep === 5 && campaignType === null)
              }
